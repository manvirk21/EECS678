...
struct Storage {
  ...
  char* sentence[7]; // !!grew array from 6 to 7 to account for NULL in echo!!
...
void echoohce(char** strs) {
  ...
  for (iter = strs; *iter != NULL; ++iter)
    printf("%s ", *iter); // !!changed from iter to *iter to match the %s type!!
  ...
  // wrong.
  for (; *iter != *stop_beginning; --iter) //using stop_beginning instead of NULL
    printf("%s ", *iter);
...
int main(int argc, char** argv) {
  ...
  // !!Initialize num_bugs_on_mars here instead!!
  ...
  bug_info.sentence[6] = NULL; // !! added NULL to stop loop
  ...
  // !!Change bug_info.sentence index from 2 to 6 for output of null!!
  ...
  // No need to free bug_infor.useless_bug b/c it isn't dynamically allocated

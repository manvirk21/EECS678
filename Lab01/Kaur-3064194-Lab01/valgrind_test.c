...
int main() {
  ...
  // !!initialized the variable in the for loop!!
  for (uninitialized_variable = 0; uninitialized_variable < 100; uninitialized_variable++) {
    ...
    *definitely_lost = (void*) malloc(7); // Give the pointer something else to
                                          ...
                                          // indirectly lost.
    free(*definitely_lost); // !!frreing the memory for definitely_lost pointer!!
    free(definitely_lost); // !!freeing memory allocated for definitely_lost!!
  }
  ...
  free(still_reachable); // !!freeing memory allocated for still_reachable!!
  ...
  possibly_lost -= 4; // !!pointing to the front of the block as well!!
  free(possibly_lost); // !!freeing memory allocated for possibly_lost!!

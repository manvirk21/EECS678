...
int main(int argc, char *argv[])
{
  ...
  char * argbuf=(char *)malloc(10); //allocate dynamic memory on the heap to hold a string
  ...
  if (pid_4 == 0) {
    ...
    strcpy(argbuf,"--lines=");  //copy first string
    strncpy(argbuf+8,argv[3],1); //copy second character at offset 8

    execl(HEAD_EXEC,HEAD_EXEC,argbuf,(char*)NULL);
    exit(0);
  }

  // make sure we are closing all the pipe
  close(p1[0]);
  close(p1[1]);
  close(p2[0]);
  close(p2[1]);
  ...
  free(argbuf); // deallocate dynamic memory
